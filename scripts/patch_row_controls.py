...
def patch_fast_final():
  ...
  function decorate(base){base=base||root();base.querySelectorAll('.a4-title,.a4-subtitle,.a4-section-title').forEach(function(n){n.setAttribute('contenteditable','true');n.setAttribute('spellcheck','false');});base.querySelectorAll('.a4-page .a4-content table').forEach(function(t){if(!isDocTable(t))return;t.classList.add('a4-scope-table');Array.prototype.slice.call(t.querySelectorAll('thead th')).forEach(function(th){th.setAttribute('contenteditable','true');th.setAttribute('spellcheck','false');});ensureDeleteButtons(t);if(!t.classList.contains('a4-equipment-checklist-table')){var next=t.nextElementSibling;if(!(next&&next.classList&&next.classList.contains('a4-row-control-bar'))){var bar=document.createElement('div');bar.className='a4-row-control-bar a4-no-print';bar.innerHTML='<button type="button" class="a4-fast-add-row-btn">+ Add Row</button>';t.parentNode.insertBefore(bar,t.nextSibling);}}});applyEqWidths(base);updateFooters(base);}"""
